...
export const TransactionModal: React.FC<TransactionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedPortfolioId) {
      setError('Please select a valid portfolio account');
      return;
    }
    setError(null);
    setIsLoading(true);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase">
            Target Account / Broker
          </label>
          <select
            value={selectedPortfolioId}
            onChange={(e) => setSelectedPortfolioId(e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs font-semibold bg-white focus:ring-2 focus:ring-emerald-500"
          >
            {portfolios.map((p) => (
              <option key={p.id} value={p.id}>
                {p.name} (Available Cash: PKR {p.cash_balance.toLocaleString()})
              </option>
            ))}
          </select>
        </div>
        ...
        <Input
          ...
          placeholder="e.g. Trade order ID or reference"
          value={notes}
